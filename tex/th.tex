...
Podjęto decyzję o zaprojektowaniu i wykonaniu prototypu systemu, który montowany będzie na maszynach rozbijających jaja firmy OVO-TECH.
Omówiono konstrukcję tego systemu, algorytmy, jakie są w nim zaimplementowane, a następnie wykonano i przetestowano prototyp.\\
...
\begin{description}
  ...
  \item[Thirdly,]the image is segmentated, and the shape recognition methods are used to detect existence, position and size of the egg yolk.
...
To reduce computation time and false detection rate, minimum distance between image centers is given as a parameter \textbf{minimum distance}.
...
It is stored in detector.py file, and automatically runs when system starts. It has been achieved by appending a "sudo python detector.py" line at the end of /etc/rc.local file.


Firstly, used Python libraries are loaded. picamera is used for comunication with Raspberry Camera; numpy, itertools and cv2 are used for image matrices processing; os handles I/O operations, and time is used when performance is measured.
